...
def sign_hash(hash_value, private_key):
    ...
    print(f"Signature = (Hash mod n)^d mod n = {small_hash}^{d} mod {n} = {signature}")

    return signature, small_hash

def verify_signature(signature, hash_mod_n, public_key):
    """Verify a signature using RSA: H' = S^e mod n, then check if H' = H mod n."""
    n, e = public_key
    ...
    print(f"Recovered hash = Signature^e mod n = {signature}^{e} mod {n} = {recovered_hash}")
...
def main():
    ...
    recovered_hash = verify_signature(signature, hash_mod_n, public_key)
