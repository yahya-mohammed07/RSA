...
def string_to_decimal(input_string):
    # Convert string to bytes and then to hexadecimal
    hex_representation = binascii.hexlify(input_string.encode())
    # Convert hexadecimal to decimal
    decimal = int(hex_representation, 16)
    return decimal
...
def encrypt_hash(hash_value, public_key):
    ...
    hash_bytes = hash_value.to_bytes((hash_value.bit_length() + 7) // 8, byteorder='big')
